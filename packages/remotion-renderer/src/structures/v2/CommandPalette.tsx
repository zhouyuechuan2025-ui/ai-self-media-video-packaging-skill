  ...
  return (
    <AbsoluteFill data-structure-identity="command-palette" data-critical-content="true" style={typography}>
      <SideSurface side={side} palette={palette}>
        ...
        <div style={{position: 'absolute', right: 24, bottom: 24, padding: '10px 13px', borderRadius: 10, background: palette.accent, color: palette.canvas, fontSize: 16, fontWeight: 950, letterSpacing: '0.08em', transform: `scale(${0.75 + phase(progress, 0.72, 1) * 0.25})`}}>{content.resultState}</div>
      </SideSurface>
    </AbsoluteFill>
  );
};
