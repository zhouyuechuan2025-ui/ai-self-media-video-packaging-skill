...
export const BeforeAfterScrub = ({content, progress, palette}: StructureProps): ReactElement => {
  ...
  const scrub = 18 + phase(progress, 0.12, 0.65) * 34;
  return (
    <AbsoluteFill data-structure-identity="before-after-scrub" data-critical-content="true" style={typography}>
      <FullScreenSurface mode="opaque" palette={palette}>
        <div style={{position: 'absolute', inset: '10% 7% 13%', borderRadius: 30, overflow: 'hidden', border: `2px solid ${palette.line}88`, background: palette.surface}}>
          ...
          <div style={{position: 'absolute', inset: 0, padding: '7%', background: palette.card, clipPath: `inset(0 0 0 ${scrub}%)`}}><div style={{marginLeft: '52%', textAlign: 'right'}}><Kicker color={palette.accent}>AFTER</Kicker><div style={{marginTop: 120, fontSize: 62, lineHeight: 1, fontWeight: 950, color: palette.foreground}}>{content.after}</div></div></div>
