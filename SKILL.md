...
Turn an existing center-presenter talking-head video and SRT into a reproducible visual-packaging project. Keep the source edit, voice, and burned-in captions intact unless the user explicitly requests otherwise.
...
2. Present the plan first and wait for the user to confirm it. Do not continue until the user approves that same job's Gate A; silence, a prior job's approval, or general permission is not approval.
...
- For a 16:9 center-presenter source, use left/right lanes at x=6-32% and x=68-94%; protect the center x=35-65% and the bottom 18% burned-in subtitle band.
- Reserve full-screen coverage for hooks, chapter bridges, evidence, payoff, or CTA. Ordinary full-screen moments last about 1-2 seconds before returning to the presenter.
- Use semantic palettes rather than random recoloring: impact, gradient keyword, signal route, editorial stamp, completion rail, and paper-sketch families must remain visually distinct.
...
- Do not repeat the same structure/palette pair on adjacent beats, use one template more than three times, or place two ordinary overlays on the same side consecutively.
